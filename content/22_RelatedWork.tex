...
co-schedule.
A co-schedule's performance is defined as the turnaround time.
...
During the symbiosis phase, the scheduler selects the schedule, which
...
not only on high IPC, but also on usage conflicts in shared hardware
...
As modern \gls{intel} multi-cores use \gls{ht} in each physical core, symbiotic
...
a shared L2 cache per chip.
They try to spread memory intensive tasks as much as possible,
...
DI uses the pain metric derived offline from the memory-reuse patterns,
...
core at least once.
So, the observation thread on a fast core searches for a suitable thread on a slow
core.
...
Their implementation takes an application as input, starts it and
...
notebooks or smartphones, and also in datacenters, as less energy usage mean less
...
The CPU cache proved to be a major side channel to deduce private
cryptographic keys.
...
executes a different branch, depending on the value of the current
...
\begin{figure}[h]
  ...
  \begin{captionbeside}[Intel Haswell generation cache inclusiveness.]{The \gls{llc} of \gls{intel} Haswell generation
...
After the right set of cache lines is found, the access frequency can be
observed.
...
Only a physical separation of the caches or access right management prevents a
PRIME+PROBE attack.
