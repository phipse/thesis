...
\todo{energy efficiency model research}
...
In \cite{POET} a mobile \gls{intel} Haswell and a Samsung Exynos 5 Octa are
...
\cite{slow down or sleep} considers server utilization below 30\%, at which
point the server's energy efficiency is worst.
\citeauthor{slow down or sleep} suggest, that low frequency/low voltage setup
provide the best energy efficiency, while meeting quality of service
parameters.
But only for web-server or video decoding like workloads.
Overall, their research suggest to avoid turbo boost and to use high sleep
states.
However, the possibilities to influence sleep states are limited for a load
balancer.
If the thread count is low, the load balancer can clear cores to allow the CPU
to go to sleep.
% todo can I derive a model. How can a load balancer affect the sleep states?



\todo{security research regarding cache attacks}
