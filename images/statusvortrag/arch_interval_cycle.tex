...
\begin{tikzpicture}[node distance=1cm]
  ...
  % Colors
  % Styles
  \tikzstyle{defaultRec} = [font=\small, text centered, inner sep = 3pt,
...
%    \draw[black,very thin] (-3cm,\y cm) -- (3cm,\y cm);
  ...
  \node(place) [defaultRec, below of=deci, anchor=north, node distance=1cm] {Placement Generator};

  \node(help) [node distance=1cm,below of=place] {};
  \node(topo) [defaultRec, inner sep = 1pt, left of=help, anchor=east,xshift=-1mm] {HW-Topology};
